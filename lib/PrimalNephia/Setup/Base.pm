package PrimalNephia::Setup::Base;
use strict;
use warnings;
use File::Spec;
...
sub new {
    ...
    my %plugin = map { 'PrimalNephia::Plugin::'.$_ => '0' } @{$opts{plugin}};
    ...
    $opts{meta_template} = PrimalNephia::MetaTemplate->new;
...
sub create {
    ...
    for my $method (keys %{$self->{additional_methods}}) {
...
sub app_class_file {
    ...
    my @plugins = sort {
        ...
        (my $module = $_) =~ s/^PrimalNephia::Plugin:://; $module
    } grep {
        $_ =~ /^PrimalNephia::Plugin::/
...
sub cpanfile {
    ...
    my $required_modules = join "\n", map {
        ...
        ($b =~ /PrimalNephia/) <=> ($a =~ /PrimalNephia/) || $a cmp $b
    } keys %{$self->{required_modules}};
...
<head>
  ...
  <title>[= title =] - powerd by PrimalNephia</title>
</head>
<body>
  <div class="title">
  ...
  <div class="content">
    <h2>Hello, PrimalNephia world!</h2>
    <p>PrimalNephia is a mini web-application framework.</p>
    <pre>
    ### [= apppath =]
    use PrimalNephia;
    ...
    <h2>See also</h2>
    <ul>
      <li><a href="https://metacpan.org/module/PrimalNephia">Read the documentation</a></li>
    </ul>
  </div>

  <address class="generated-by">Generated by PrimalNephia</address>
</body>
</html>
